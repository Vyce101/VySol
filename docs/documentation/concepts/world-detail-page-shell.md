...
The frontend starts the World Detail shell and reads mode plus optional draft ID values from route/query state. If the mode is `draft` and a draft ID is present, the shell asks the backend for draft detail so the visible state reflects backend-owned splitter defaults and staged-source setup. If draft mode has no draft ID, the shell keeps a draft-safe visual fallback for direct development loading without creating a frontend-only committed draft.
...
World Detail Page Shell currently receives frontend route/query state for display mode and optional draft ID. In backend-backed draft mode, it also receives draft detail from the Draft World Detail API. It imports app-owned built-in assets for the visual default background, font, and logo mark.
...
- Rendering the shell must not create a draft; draft creation belongs to a future Create World entrypoint calling the backend API.
