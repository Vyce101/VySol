...
World Detail Page Shell is VySol's first frontend layout contract for a single world screen. It provides the visual frame that future Customize and Ingestion features will fill without making the shell responsible for those feature behaviors.
...
- Switching between empty Customize and Ingestion placeholder panes.
...
- Backend world-detail routes or API contracts.
...
The frontend starts the World Detail shell and reads a simple mode value from route/query state. If the mode is `draft`, the shell renders draft-safe placeholder pane labels. If the mode is `committed`, the same shell renders committed-world placeholder pane labels.

The page always renders the same top-level frame: background image, dark cinematic overlays, non-clickable VySol branding, and the centered tab control. The user can switch between `Customize` and `Ingestion`, but each tab currently shows only a placeholder pane because feature controls belong to later tickets.

## Inputs

World Detail Page Shell currently receives only frontend route/query state for display mode. It also imports app-owned built-in assets for the visual default background, font, and logo mark.

It does not receive world records, source lists, ingestion attempts, saved settings, database rows, provider responses, or uploaded assets.

## Outputs

The system produces visible frontend UI state only. It renders the page shell, active tab state, and placeholder pane copy.

It does not write files, create database rows, call backend APIs, persist draft data, start ingestion, save settings, or emit app logs.
...
The `Customize` and `Ingestion` tabs are visible in both draft and committed modes. The panes intentionally stay empty except for minimal placeholder copy until the related feature tickets add real controls.
...
It must stay separate from backend storage, draft-world registries, committed-world indexes, source staging, ingestion attempt state, parsing, chunking, graph systems, and retrieval.
